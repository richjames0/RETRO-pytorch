...
class RMSNorm(nn.Module):
    def __init__(self, dim, *, eps=1e-8, gated=False):
        super().__init__()
        self.eps = eps
        self.scale = dim**-0.5
            ...
            self.layers.append(
                nn.ModuleList(
                    [
                        ...
                        wrapper(
                            Attention(dim=dim, context_dim=context_dim, dim_head=dim_head, heads=heads, dropout=attn_dropout)
                        )
                        if has_cross_attn
                        else None,
    ...
    def forward(self, x, *, encoder=None, encoder_retrieved_mask=None, context_mask=None, retrieved=None):
        ...
        if exists(retrieved):
            ...
            cross_attn_q_pos_emb = self.rotary_pos_emb(
                self.chunk_size, device=device, offset=self.chunk_size - 1
...
class RETRO(nn.Module):
    ...
    def forward(self, seq, retrieved=None, return_loss=False):
        ...
        assert (
            chunk_size >= self.chunk_size
        ), 'chunk size of retrieval input must be greater or equal to the designated chunk_size on RETRO initialization'

        num_seq_chunks = n // self.chunk_size
        assert (
            num_chunks == num_seq_chunks
        ), f'sequence requires {num_seq_chunks} retrieved chunks, but only {num_chunks} passed in'
